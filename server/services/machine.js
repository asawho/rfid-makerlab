...
class Machine {
    ...
    //Map just the fields for the client
    toClientVM() {
    ...
    flip (user, enabled) {
        if (this.type === 'piserver') {
            ...
            //Ignore for now just so goes through
            //throw "Unknown machine type " + this.type + " for machine id " + this.id;
        }

        //Skip setting a new time if they guy is just turning it back on
        if (enabled && this.inUse && this.inUseBy==user) return;
        this.inUse = enabled;
        this.inUseBy = user;
